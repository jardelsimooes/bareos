...
# Kernel >= 5.8.x
# Testado e homologado para a versão do BareOS
#
...
RELEASE="http://download.bareos.org/bareos/release/20/xUbuntu_20.04/"
USER="admin"
PASSWD="bareos"
PROFILE="webui-admin"
...
#
# Verificando se o usuário é Root, Distribuição é >=20.04 e o Kernel é >=5.8 <IF MELHORADO)
...
if [ "$USUARIO" == "0" ] && [ "$UBUNTU" == "20.04" ] && [ "$KERNEL" == "5.8" ]
	then
		...
		echo -e "Kernel é >= 5.8, continuando com o script..."
		sleep 5
	else
		echo -e "Usuário não é Root ($USUARIO) ou Distribuição não é >=20.04.x ($UBUNTU) ou Kernel não é >=5.8 ($KERNEL)"
...
#
echo
echo -e "Instalação do BareOS no GNU/Linux Ubuntu Server 20.04.x\n"
